...
### 问题三（多时刻预报下的滚动合同调整，主方案 B1）

| 指标 | 数值 |
|---|---|
| 评价期 | 2025.2.1–12.31，334 天，2/1 起评 10800 kWh（与问题二一致） |
| 2×2 费用矩阵 | A0 **1395.6995** / A0K **1376.2039** / B0 **1355.5778** / B1 **1345.7069** 万元 |
| 主方案 B1 构成 | 合同费 1311.5237 + 调整附加费 15.0544 + 紧急费 19.1287 万元 |
| 分解 | 调整机会 −40.12、0:00 预报通道 −19.50、联合 −49.99、重叠 9.62 万元 |
| 四个指定日期 | 2025.3.20 / 6.21 / 9.23 / 12.21 的表 1、表 2、表 3 见 `results/q3_tables.json` |
| 校验 | 平衡残差 ≤1.2e-13、充放电重叠 0、跨日断裂 0、SOC ∈ [1200, 10800]、功率未越界 |

`result3.xlsx` 分「计划购电量 / 调整购电量 / 充放电量 / 紧急购电量」四张表。
...
| `q3_core.py` | **问题三模型链**：整点预报区间化与端点锚定、κ_g 分档收缩融合、日前计划 LP、带双向违约价的调整层 LP、尽限执行与全年回放 | 被 `q3_finalize.py` 调用 |
| `q3_finalize.py` | **问题三唯一入口**：复现 2×2 费用矩阵 + 论文表1/2/3 + 四个指定日期结果 + 写 result3.xlsx | `result3.xlsx`、`q3_tables.json` |
...
python scripts/q3_finalize.py         # 问题三：2×2 矩阵 + 四日期表 + result3.xlsx
```
