...
\begin{enumerate}
  ...
  \item \textbf{计划层}（\S\ref{sec:q2-lp}）：以 $\widehat N^{(d)}$ 与日初电量 $E_0^{(d)}$ 求解日计划线性规划，输出计划购电量 $b_t$（提交结算）；计划充电量 $c_t$ 仅供分析，不约束执行；
...
\textbf{（三）分位裕量。}前两项只给出条件中心趋势，残余随机波动仍以 5 倍价惩罚。对每个时段 $t$，取同一窗口内净负载相对窗口中位数的 $\beta$ 上尾分位数作为预留：
\begin{equation}
  M_{\beta,t}=Q_\beta\Bigl\{\,N_t^{(s)}-\operatorname*{median}_{s'\in[d-W,\,d)}N_t^{(s')}\,\Bigr\}_{s\in[d-W,\,d)}
  \;=\;Q_\beta\bigl\{N_t^{(s)}\bigr\}_{s\in[d-W,\,d)}-\,m^{\mathrm{r}}_{d,t}\ .
\end{equation}
即裕量只刻画窗口内净负载的离散程度，与星期偏差校正项互不包含。其经济逻辑与报童模型一致~\cite{newsvendor}：少计划 1 单位电量需以 $5p$ 紧急补购、相比正常计划多付 $4p$，多计划 1 单位则白付 $p$，故最优计划量应落在需求分布的某个上分位数，单时段简化情形下临界分位数为 $4p/(4p+p)=0.80$；但储能提供跨时段物理缓冲，改变了边际代价结构，$\beta$ 不直接套用理论值，而由回放择优（\S\ref{sec:q2-walkforward}）。

\textbf{（四）冷启动回退。}当 $d<30$（1 月内）时，历史不足以估计上述三项，预测器整体回退为附件 1 的代表日净负载曲线。需要如实说明一个实现细节：预测器自 $d=30$ 启用后，星期偏差的历史候选日可能落在 1 月早期；对这些可用历史不足 7 天的候选日，中位数辅助量返回零向量，即该日以自身实测充当``当日减近期水平''进入星期偏差的平均。这一影响随候选日后移在 2 月中旬后自然消失，本文不以后处理加以掩盖。
...
\begin{itemize}
  \item $\mathrm{net}\ge0$（有盈余）：在储能可充空间与功率上限之内\textbf{尽可能充入}，即 $c_t=\min\{\mathrm{net},\ 5000/6,\ (E_{\max}-E_t)/\eta_c\}$，充不进的部分为弃置 $w_t$；
  \item $\mathrm{net}<0$（有缺口）：在储能可放空间与功率上限之内\textbf{尽可能放出}，即 $d_t=\min\{-\mathrm{net},\ 5000/6,\ (E_t-E_{\min})\,\eta_d\}$，仍不足的部分即为紧急购电 $e_t$。
\end{itemize}
该规则只看当前区间的盈余、功率与储电量界，\textbf{不受计划充电意图约束}；有缺口时即时释放可用电量，可直接降低 5 倍电价的紧急购电量；且完全因果，仅使用当前区间的实测值与当前储电量。执行层不允许以普通电价补购未计划电量，与题意的结算口径一致。
